...
class MultiplayerOptions(ndb.Model):
	ndb_mode   = msgprop.EnumProperty(NDB_MultiGameType, default=NDB_MultiGameType.SIMUSOLO)
...
class SeedGenParams(ndb.Model):
	...
	def flag_line(self, verbose_paths=False):
		...
		if self.warmth.enabled:
			flags.append("Frags/%s/%s/%s/%s/%s/%s" % (self.warmth.count,self.warmth.key_1, self.warmth.key_2, self.warmth.key_3, self.warmth.required, self.warmth.tolerance))
